...
describe('preTickedNative', () => {
  const candidates: NativeCandidate[] = [
    ...
    { key: 'vendorx-grok-4.5', gateway: 'vendorx', id: 'grok-4.5', contextWindow: 128000, baseUrl: 'https://bifrost.advai.net/v1' },
  ];
...
describe('deriveInitState', () => {
  ...
  it('derives a detected gateway provider and harness', () => {
    const state = deriveInitState(config({ m: { gateway: 'vendorx', id: 'm' } }), 'project', [
      { harness: 'opencode', provider: 'vendorx', count: 1, key: 'opencode/vendorx' },
    ]);
    expect(state.providerKeys).toEqual(['opencode/vendorx']);
...
describe('oauthProvidersFor', () => {
  const refs = [
    ...
    { harness: 'opencode' as const, provider: 'vendorx', id: 'deepseek', ref: 'vendorx/deepseek' },
  ];
  const none = () => null;
  const some = () => ({});
  ...
  it('leaves api-key providers alone', () => {
    ...
    expect(got.has('vendorx')).toBe(false);
  });
});
...
describe('nativeCandidatesFrom — models the router cannot reach', () => {
  ...
  it('every candidate it returns survives parseConfig', () => {
    const refs = [
      ...
      { harness: 'opencode' as const, provider: 'vendorx', id: 'deepseek-v4', ref: 'vendorx/deepseek-v4' },
    ];
