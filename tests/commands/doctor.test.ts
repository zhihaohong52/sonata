...
`);
    ...
    try {
      ...
      expect(checks.find((c) => c.name === 'key source: vendorx')).toEqual({
        ...
        detail: 'vendorx: credential from sonata\n  ! vendorx: no credential from sonata — ' +
          'run `sonata auth add vendorx`',
      });
    } finally {
      globalThis.fetch = originalFetch;
    }
...
`);
  ...
  it('reports native serve health and key source without exposing key values', async () => {
    ...
    try {
      ...
      expect(keyChecks.find((c) => c.name === 'key source: vendorx')).toEqual({
        name: 'key source: vendorx', ok: true, detail: 'from sonata',
      });
