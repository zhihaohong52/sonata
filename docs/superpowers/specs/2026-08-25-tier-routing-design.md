...
gateway = "vendorx"              # native route (default execution path)
...
(`opencode-vendorx-deepseek-v4-flash-0731`) become `[models]` entries with
...
- **Normalization**: `normalizeModelName` strips harness/provider prefixes
  and date suffixes (`vendorx-deepseek-v4-flash-0731` → `deepseek-v4-flash`),
