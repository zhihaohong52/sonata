...
gateway = "vendorx"                    # which gateway serves it
...
base_url = "https://bifrost.advai.net/v1"   # discovered from opencode at init, editable
...
- **Key check** — per chosen gateway, resolve the credential chain and show
  the source by name (`vendorx: key from opencode`), or offer inline
