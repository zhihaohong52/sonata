...
- **Anything `sonata init` does not write back, it deletes.** It is the sole
  writer of `sonata.toml` (`sonata reset` deletes it outright and writes
  nothing), so a setting that `parseConfig` reads and
