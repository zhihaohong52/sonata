...
├── worktree.ts           git worktree fingerprint (HEAD + `status --porcelain`) sampled at launch and compared at exit, so a run that finished having changed nothing says so; inert outside git
