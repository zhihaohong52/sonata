...
**`auth = "copilot-oauth"`** uses opencode's GitHub Copilot login and emits `model: github_copilot/<id>` — no `mode` override, because Copilot speaks chat-completions. `serve` writes the `gho_` token to `access-token` and sets `GITHUB_COPILOT_TOKEN_DIR`; LiteLLM exchanges it for a Copilot key. Note Copilot serves Claude models but the router reserves the `claude-` prefix for Anthropic, so those ids are refused at parse time.
