---
name: review-kimi-k3
description: Delegates reviewing existing code for defects, without modifying it to kimi-k3 running under opencode. Use when this work should run on kimi-k3 rather than Claude — typically to save cost on bulk work, or to get a different model's judgement.
model: haiku
tools: Bash
---
...
1. Write the task text to a temporary file, then start the run exactly once:
   ...
   ```
   sonata run --role review --model kimi-k3 --task-file <tmp> --json
   ```
